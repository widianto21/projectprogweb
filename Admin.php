	...
	class Admin extends CI_Controller{
		...
		function input_data_dokter($mod = 0){
			...
			if($this->form_validation->run() == FALSE){
				...
				if($mod != 0){
					$cond1 = array('col' => 'ID_DOKTER', 'cond' => $data['data_dokter']['ID_DOKTER']);
					$cond2 = array('col' => 'ID_USER', 'cond' => $data['data_user']['ID_USER']);
		...
		function update_data_jadwal(){
			...
			if($this->form_validation->run() == FALSE){
				...
				$cond = array('col' => 'ID_JADWAL', 'cond' => $data['ID_JADWAL']);
